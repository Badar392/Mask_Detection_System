...
is not read from a server-side `cv2.VideoCapture(0)` device. The webcam tab uses WebRTC. Frames are decoded and processed in Python memory
by the same human-face and mask pipeline as uploads; snapshots and intermediate
image files are not saved.
